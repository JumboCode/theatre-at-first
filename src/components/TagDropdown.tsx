...
//TODO implement "x" on tags and figure out behavior if there are a lot of tags
            ...
            </div>
            {
                display && (
                    <div className="border-2 h-[144px] rounded-xl overflow-auto">
                        {props.tags.map((tag) => (
                            <div key={tag} onClick={() => handleSelect(tag)} className="pl-2 hover:bg-violet-600 hover:font-bold active:bg-violet-700 py-2">
                                <p>{tag}</p>
                            </div>
                        ))} 
                    </div>
